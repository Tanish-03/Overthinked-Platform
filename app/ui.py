...
def _header():
    ...
    st.markdown(f"<p style='text-align:center' class='muted'>{TAGLINE}</p>", unsafe_allow_html=True)
    st.write("")
...
def _support_tab():
    ...
    if ask and st.session_state.user_input.strip():
        with st.spinner("Thinking…"):
            ok, data = get_ai_response(st.session_state.user_input)
        if ok:
...
def _trends_tab():
    ...
    st.markdown("""
    **Future DE idea:** (consented data)
    - Store in S3 → process with Lambda/Glue → query with Athena → visualize in QuickSight/Streamlit.
    """)
...
def render_ui():
    ...
    with tab1: _support_tab()
    with tab2: _trends_tab()
